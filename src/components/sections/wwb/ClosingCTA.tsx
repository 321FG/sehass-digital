...
const invertedOutline = cn(
  ...
  "transition-colors duration-150 hover:bg-white/10 hover:border-white/40",
  "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-brand",
);
...
export async function ClosingCTA({
  ...
  const resolvedHref = href ?? `/${lang}/contact`;
  return (
    <Section tone="brand" spacing="lg" aria-labelledby="closing-cta-heading">
      <Container size="md">
        <div className="flex flex-col items-start gap-8 text-left md:items-center md:text-center">
          <span className="inline-flex items-center gap-2 text-xs font-medium uppercase tracking-[0.16em] text-on-brand-muted">
            ...
            {dict.sections.letsBuildEyebrow}
          </span>
          <h2
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <NextLink href={resolvedHref} className={invertedPrimary}>
              {cta}
              <svg
                aria-hidden
                viewBox="0 0 20 20"
            ...
            {secondary ? (
              <NextLink href={secondary.href} className={invertedOutline}>
                {secondary.label}
              </NextLink>
            ) : null}
          </div>
